...
from config import RIOT_API_KEY, TIER_SEED, TIER_SCORE
from cogs.economy import add_coins
from cogs.shared_views import ConfirmView, _display_name, CaptainCoinflipView, DraftView, CreateTournamentView, ReportView, DraftSetupView, CaptainModeView
import aiohttp
import asyncio
import random
...
class Tournament(CogBase):
    ...
    async def captain_draft_cmd(
        ...
        # ── Inner function: start draft flow after captains determined ──
        async def start_draft_flow(cap_a_info, cap_b_info, is_random):
            captains_info = [
                {
